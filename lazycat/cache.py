"""
Lightweight TTL response cache with bounded LRU eviction.

Caches function results in-memory with a configurable TTL. Entries are grouped
so write paths can invalidate everything derived from the data they changed.

Usage:
    from lazycat.cache import timed_cache, invalidate_cache
...
The store is module-global by design: importers share one cache, so a single
invalidate_cache() call clears every decorated function in that group.
"""
...
def clear_cache() -> None:
    ...
    Intended for tests and for callers that need a hard reset; prefer
    invalidate_cache(group) in application code.
    """
    _cache.clear()
    _stats.update({"hits": 0, "misses": 0, "invalidations": 0})
